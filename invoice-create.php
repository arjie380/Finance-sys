...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'create_invoice') {
    ...
    $response = array('status' => 'error', 'message' => 'There was an issue processing the invoice.');
    ...
    $customer_address_1 = isset($_POST['customer_address_1']) ? $_POST['customer_address_1'] : '';
    $customer_town = isset($_POST['customer_town']) ? $_POST['customer_town'] : '';
    $customer_postcode = isset($_POST['customer_postcode']) ? $_POST['customer_postcode'] : '';
    ...
    $customer_county = isset($_POST['customer_county']) ? $_POST['customer_county'] : ''; // Country
    $customer_county_ship = isset($_POST['customer_county_ship']) ? $_POST['customer_county_ship'] : ''; // Shipping country

    // Ensure all fields are sanitized
    $invoice_id_generated = generateInvoiceId();  // Generate a unique invoice ID

    // Assuming a function exists to insert invoice details into your database.
    $invoice_created = createInvoice($invoice_id_generated, $invoice_type, $invoice_status, 
                                      $invoice_date, $invoice_due_date, $customer_name, 
                                      $customer_address_1, $customer_town, $customer_postcode, 
                                      $customer_email, $customer_phone, $customer_county, 
                                      $customer_county_ship);

    if ($invoice_created) {
        // If the invoice creation was successful, send a success response.
        $response = array(
            'status' => 'success',
            'message' => 'Invoice created successfully.',
            'invoice_id' => $invoice_id_generated  // Optionally send back the created invoice ID
        );
    } else {
        // If there was an issue with invoice creation
        $response['message'] = 'Error while creating the invoice. Please try again.';
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                $('#response .message').text(response.message); // Show success message
                $('#response').show();
            } else {
                $('#response').removeClass('alert-success').addClass('alert-danger');
                $('#response .message').text(response.message); // Show error message
                $('#response').show();
            }
        },
        error: function(xhr, status, error) {
            ...
            $('#response').removeClass('alert-success').addClass('alert-danger');
            $('#response .message').text('An unexpected error occurred.');
            $('#response').show();
        }
    });
});
